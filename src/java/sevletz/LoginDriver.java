   ...
   @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            cc.add(Restrictions.eq("dnicz", unamez));
            cc.add(Restrictions.eq("dpasswordz", pwz));

            POJO.Driverdetails userz = (POJO.Driverdetails) cc.uniqueResult();
            if (userz == null) {
                out.write("Invalid");
            } else {
                HttpSession hs = request.getSession();
                hs.setAttribute("driver1", userz);
